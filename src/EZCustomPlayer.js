...
export default class EZCustomPlayer extends Component {
  ...
  _onPlayerCloseFloatPlayer(event) {
    if (typeof this.props.onPlayerCloseFloatPlayer === 'function') {
      this.props.onPlayerCloseFloatPlayer(...arguments);
    }
  }
  _onPlayerExpandFloatPlayer(event) {
    if (typeof this.props.onPlayerExpandFloatPlayer === 'function') {
  ...
  _onPlayerLoadingDidChange(event) {
    ...
    if (typeof this.props.onPlayerLoadingDidChange === 'function') {
      this.props.onPlayerLoadingDidChange({ isLoading: state.isLoading });
    }
  }
  ...
  formatTime(time = 0) {
    ...
    return `${formattedHours === '00' ? '' : formattedHours + ':'
