...
open class EZRNPlayerView: UIView {
  ...
  public var onPlayerCloseFloatPlayer: RCTDirectEventBlock?
  public var onPlayerExpandFloatPlayer: RCTDirectEventBlock?
  // MARK: - Life
...
extension EZRNPlayerView {
  open func addObservers(){
    ...
    //悬浮关闭
    NotificationCenter.default.addObserver(self, selector: #selector(self.playerCloseFloatPlayer(_:)), name: NSNotification.Name.EZPlayerCloseFloatPlayer, object: nil)
    
    //悬浮全屏播放
    NotificationCenter.default.addObserver(self, selector: #selector(self.playerExpandFloatPlayer(_:)), name: NSNotification.Name.EZPlayerExpandFloatPlayer, object: nil)
  ...
  @objc  func playerCloseFloatPlayer(_ notifiaction: Notification) {
    guard let player = notifiaction.object as? EZPlayer ,let selfPlayer = self.player , player == selfPlayer else{
      return
    }
    self.onPlayerCloseFloatPlayer?(notifiaction.userInfo)
  }
  
  @objc  func playerExpandFloatPlayer(_ notifiaction: Notification) {
    guard let player = notifiaction.object as? EZPlayer ,let selfPlayer = self.player , player == selfPlayer else{
      return
    }
    self.onPlayerExpandFloatPlayer?(notifiaction.userInfo)
  }
